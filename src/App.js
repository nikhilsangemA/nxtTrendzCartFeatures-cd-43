...
class App extends Component {
  ...
  addCartItem = product => {
    ...
    if (findObject) {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachItemOf => {
          if (eachItemOf.id === findObject.id) {
            const addOf = eachItemOf.quantity + 1
            return {...eachItemOf, quantity: addOf}
          }
          return eachItemOf
        }),
      }))
  ...
  removeCartItem = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.filter(eachFilter => {
        if (id !== eachFilter.id) {
          return {...eachFilter}
        }
        return null
